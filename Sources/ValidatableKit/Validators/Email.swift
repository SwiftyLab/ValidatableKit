public extension Validator where T: StringProtocol {
    ...
    /// Validates whether a `String` is a valid international email address.
    static var isInternationalEmail: Validator<T> {
        .init {
            guard
                let range = $0.range(
                    of: regexInternationalEmail, options: [.regularExpression]),
                range.lowerBound == $0.startIndex
                    && range.upperBound == $0.endIndex,
                // The numbers beneath here are as defined by https://emailregex.com/email-validation-summary/
                $0.count <= 320,  // total length
