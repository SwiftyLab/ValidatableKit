...
public extension Validator where T: StringProtocol {
    ...
    static var alphanumeric: Self {
        .containsOnly(.alphanumerics)
    }

    /// Validates that all characters in a `String` are in the supplied `CharacterSet`.
    static func containsOnly(_ characterSet: Foundation.CharacterSet) -> Self {
        .init {
...
extension ValidatorResults {
    /// `ValidatorResult` of a validator that validates that a `String`
    /// contains characters in a given `CharacterSet`.
    public struct CharacterSet<S: StringProtocol> {
        ...
        /// On validation failure, the first substring of the input with
        /// characters not contained in `characterSet`.
...
extension ValidatorResults.CharacterSet: ValidatorResult {
    ...
    public var failureDescriptions: [String] {
        guard
            let slice = invalidSlice
        else {
            return [
                "(allowed: \(self.allowedCharacterString))"
            ]
        }
        return [
            "contains '\(slice)' (allowed: \(self.allowedCharacterString))"
        ]
    }
}
...
extension Validator where T == [String] {
    ...
    /// Validates that all characters in elements of a
    /// `[String]` are in the supplied `CharacterSet`.
    public static func containsOnly(
        _ characterSet: Foundation.CharacterSet
    ) -> Validator {
        .init {
...
extension ValidatorResults {
    ...
    /// that all elements of a `[String]` contain characters
    /// in a given `CharacterSet`.
    public struct CollectionCharacterSet {
        ...
        /// On validation failure, the first substring of the input with characters
        /// not contained in `characterSet`.
        var invalidRanges: [(Int, Swift.Range<String.Index>)?] {
            return self.strings.enumerated().compactMap {
                if let range = $1.rangeOfCharacter(
                    from: self.characterSet.inverted
                ) {
                    return ($0, range)
                }
                return nil
...
extension ValidatorResults.CollectionCharacterSet: ValidatorResult {
    ...
    public var failureDescriptions: [String] {
        let disallowedCharacters = self.invalidRanges.compactMap { $0 }
            .map { (invalidSlice) in
                "string at index \(invalidSlice.0) contains "
                    + "'\(String(self.strings[invalidSlice.0][invalidSlice.1]))'"
            }
        return [
            disallowedCharacters.joined(separator: ", ")
                + "(allowed: \(self.allowedCharacterString))"
        ]
    }
}
...
///
///     .containsOnly(.alphanumerics + .whitespaces)
///
